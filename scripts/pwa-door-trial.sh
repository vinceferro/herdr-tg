...
# real Unix socket with Telegram counted instead of called, connects a real bridge speaking
# hub-proto, and then spawns the REAL gateway binary — `kickoff-door`, the one program in this
# workspace that listens — on a loopback port of its own. A scripted client shaped like the
# PWA's own (its anchor poll, its SSE subscribe, its write POSTs) then proves the whole loop:
# the ask envelope arrives on the stream stamped with its conversation and lane, a POSTed
# choice reaches the bridge's socket as a `choice` frame, the POST is answered from the hub's
# own result file, a refusing bridge's word lands on the ring, a POSTed message naming a lane
# reaches that lane's session, a second conversation's answers reach it and nobody else, the
# 401 is byte-identical, replay from an old cursor is exact, and nothing on the HTTP wire —
# raw bytes, both directions — names this machine.
#
...
#
# Like the fleet trial, this is a thin wrapper: the trial itself is one `#[ignore]`d test inside
# the hub's own test module, because that is the only place a hub with a faked Telegram surface
# can be built at all. What this script adds over `cargo test` is the environment, a build of
# the gateway binary onto PATH (the test starts it BY NAME — a fixed program no inbound string
# chooses, which is why it is on the spawn guard's allowlist), and the refusals below.
#
...
THE_TRIAL=hub::tests::the_pwa_s_door_round_trip_reaches_the_hub_and_back_through_the_real_gateway
...
say "Running the hermetic PWA-door trial — one hub, the real gateway, no Telegram…"
say
...
[ "$RC" -eq 0 ] || die "the PWA-door trial failed; the lines above say which property"
