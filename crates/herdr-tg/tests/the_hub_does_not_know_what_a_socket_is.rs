...
const TRANSPORT: &str = "crates/herdr-tg/src/transport.rs";
...
const TRANSPORT_DIR: &str = "crates/herdr-tg/src/transport";
...
const HUB_DIR: &str = "crates/herdr-tg/src/hub";
...
const HUB_DIR_EXEMPT: &str = "crates/herdr-tg/src/hub/tests.rs";
...
fn a_file_that_no_longer_holds_what_it_is_named_for_fails_rather_than_scanning_a_stranger() {
    ...
    let src = dir.path().join("crates/herdr-tg/src");
...
fn a_socket_moved_into_a_module_beside_the_hub_is_found_there_too() {
    ...
    std::fs::write(
        dir.path().join("crates/herdr-tg/src/hub/door.rs"),
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/herdr-tg/src/hub/door.rs:1")
...
fn a_symlinked_directory_under_the_hub_is_refused_rather_than_walked_past_in_silence() {
    let dir = a_tree_whose_seam_holds();
    let elsewhere = dir.path().join("crates/herdr-tg/src/hub_parts");
    ...
    std::os::unix::fs::symlink(
        "../hub_parts",
        dir.path().join("crates/herdr-tg/src/hub/parts"),
    )
...
fn a_transport_split_into_modules_is_scanned_like_the_one_file_it_grew_out_of() {
    let dir = a_tree_whose_seam_holds();
    std::fs::create_dir_all(dir.path().join("crates/herdr-tg/src/transport"))
        .expect("a transport module directory");
    std::fs::write(
        dir.path().join("crates/herdr-tg/src/transport/gateway.rs"),
    ...
    assert!(
        problems.iter().any(
            |p| p.starts_with("crates/herdr-tg/src/transport/gateway.rs:1") && p.contains("claim")
        ),
...
fn hub_at(root: &Path, commit: &str) -> String {
    ...
    assert!(
        out.status.success(),
        "git could not read {HUB} at {commit}: {}",
