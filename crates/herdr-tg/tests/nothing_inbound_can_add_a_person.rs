...
//! own list comes from the configuration, and a project's from `herdr-tg allow <repo> <user>`.
...
fn no_message_tap_or_command_can_add_a_person_to_any_list() {
    ...
    assert!(
        ...
        "the setter that lets a person speak is named outside the terminal-only door: \
         {named_it:?}. Only `herdr-tg allow <repo> <user>` at a keyboard may add a person; a \
