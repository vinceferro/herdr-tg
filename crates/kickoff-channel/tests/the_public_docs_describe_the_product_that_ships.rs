...
fn the_rates_the_readme_states_are_the_constants_the_binary_ships() {
    let per_minute = shipped_constant("crates/kickoff-channel/src/queue.rs", "PER_MINUTE");
    let reserved = shipped_constant(
        "crates/kickoff-channel/src/queue.rs",
        "RESERVED_FOR_THE_OPERATOR",
    );
    let reactions = shipped_constant(
        "crates/kickoff-channel/src/queue.rs",
...
fn subcommands() -> Vec<String> {
    let names = subcommands_in(&source("crates/kickoff-channel/src/lib.rs"));
    assert!(
        names.len() >= 10,
