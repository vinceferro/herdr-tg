...
const MAY_START_A_PROCESS: [MayStartAProcess; 4] = [
    MayStartAProcess {
        file: "crates/kickoff-channel/src/cmd/enroll.rs",
        ships: Ships::Yes {
            sites: 1,
    ...
    MayStartAProcess {
        file: "crates/kickoff-channel/src/presence.rs",
    ...
    MayStartAProcess {
        file: "crates/kickoff-channel/src/summarize.rs",
        ships: Ships::OnlyUnderCfgTest,
    ...
    MayStartAProcess {
        file: "crates/kickoff-channel/src/hub/tests.rs",
        ships: Ships::WholeFileGatedAt {
            declared_in: "crates/kickoff-channel/src/hub.rs",
...
const THE_INBOUND_PATH: [&str; 15] = [
    ...
    "crates/kickoff-channel/src/conversations.rs",
    "crates/kickoff-channel/src/gateway.rs",
    "crates/kickoff-channel/src/heartbeat.rs",
    "crates/kickoff-channel/src/hub.rs",
    "crates/kickoff-channel/src/hub/answers.rs",
    "crates/kickoff-channel/src/hub/door.rs",
    "crates/kickoff-channel/src/media.rs",
    "crates/kickoff-channel/src/presence.rs",
    "crates/kickoff-channel/src/queue.rs",
    "crates/kickoff-channel/src/registry.rs",
    "crates/kickoff-channel/src/render.rs",
    "crates/kickoff-channel/src/summarize.rs",
    "crates/kickoff-channel/src/surface.rs",
    "crates/kickoff-channel/src/transport.rs",
];
...
fn a_tree_that_starts_only_what_it_should() -> tempfile::TempDir {
    ...
    write_at(
        root,
        "crates/kickoff-channel/src/hub/tests.rs",
    ...
    write_at(
        root,
        "crates/kickoff-channel/src/cmd/enroll.rs",
...
fn a_spawn_planted_in_the_hub_is_reported_rather_than_walked_past() {
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/src/hub.rs:4")
                && p.contains("Command::new")),
...
fn a_spawn_planted_in_a_module_beside_the_hub_is_found_there_too() {
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/src/hub/door.rs:2")),
...
fn a_program_built_from_a_value_is_refused_even_in_a_file_that_may_spawn() {
    ...
    assert!(
        problems.iter().any(
            |p| p.starts_with("crates/kickoff-channel/src/cmd/enroll.rs:2")
...
fn an_allowed_file_that_has_been_renamed_fails_rather_than_scanning_nothing() {
    let dir = a_tree_that_starts_only_what_it_should();
    let src = dir.path().join("crates/kickoff-channel/src/cmd/enroll.rs");
    let moved = dir
        .path()
        .join("crates/kickoff-channel/src/cmd/enrolment.rs");
    std::fs::rename(&src, &moved).expect("rename the allowed file");
    let problems = scan(dir.path());
    assert!(
        problems.iter().any(
            |p| p.starts_with("crates/kickoff-channel/src/cmd/enroll.rs")
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/src/cmd/enrolment.rs")),
...
fn an_inbound_file_that_has_been_renamed_fails_rather_than_going_unwatched() {
    let dir = a_tree_that_starts_only_what_it_should();
    std::fs::rename(
        dir.path().join("crates/kickoff-channel/src/queue.rs"),
        dir.path().join("crates/kickoff-channel/src/pacer.rs"),
    )
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/src/queue.rs")),
...
fn a_fourth_shipped_call_site_in_enrolment_is_reported_rather_than_counted_in() {
    let dir = a_tree_that_starts_only_what_it_should();
    let path = dir.path().join("crates/kickoff-channel/src/cmd/enroll.rs");
...
fn a_spawn_that_leaves_its_test_block_stops_being_excused() {
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/src/presence.rs:2")),
...
fn a_test_module_that_stops_being_gated_takes_its_programs_into_the_binary_and_is_said_so() {
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/src/hub.rs") && p.contains("mod tests;")),
...
fn an_exemption_whose_reason_has_gone_is_refused_rather_than_left_standing() {
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/src/presence.rs")
...
fn code_pulled_into_a_crate_from_a_file_this_scan_does_not_walk_is_refused() {
    for directive in [
        ...
        let dir = a_tree_that_starts_only_what_it_should();
        let path = dir.path().join("crates/kickoff-channel/src/bot.rs");
        ...
        assert!(
            problems
                .iter()
                .any(|p| p.starts_with("crates/kickoff-channel/src/bot.rs")),
...
fn a_target_pointed_somewhere_this_scan_does_not_read_is_refused() {
    ...
    assert!(
        problems
            .iter()
            .any(|p| p.starts_with("crates/kickoff-channel/Cargo.toml") && p.contains("[[bin]]")),
