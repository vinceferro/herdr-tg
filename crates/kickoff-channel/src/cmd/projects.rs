//! `kickoff-channel projects` — every enrolled project, for a person at the keyboard or for a machine.
//!
...
pub(crate) struct Row<'a> {
    ...
    /// The people let into THIS project's conversations with `kickoff-channel allow`, as Telegram user
...
/// `kickoff-channel projects`, either shape.
...
fn projects_in(
    ...
    if !any {
        writeln!(
            out,
            "Nothing is enrolled yet. Add a project with:  kickoff-channel enroll <repo>"
        )?;
    }
    Ok(())
}
