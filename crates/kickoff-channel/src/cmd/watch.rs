//! `kickoff-channel watch` — decode the event stream for one pane.
...
async fn follow(stream: &mut herdr_client::EventStream) -> anyhow::Result<()> {
    loop {
        match stream.next().await {
            None => {
                eprintln!(
                    "kickoff-channel: herdr closed the event stream (this client does not reconnect)"
                );
                return Ok(());
            }
            ...
            Some(Err(err)) => {
                eprintln!("kickoff-channel: undecodable frame, stream continues: {err}")
            }
