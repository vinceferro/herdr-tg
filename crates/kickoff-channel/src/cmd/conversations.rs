//! `kickoff-channel open <repo>`, `grant <repo> --rooms N`, `adopt-secrets [--apply]` and
...
/// `kickoff-channel open <repo>`: a conversation for a repo, with nothing written into the repo.
...
fn open_in(
    ...
    if !created {
        writeln!(
            out,
            "\nNothing was changed. To rotate its secret, enrol it again:  kickoff-channel enroll  — a \
...
/// `kickoff-channel grant <repo> --rooms N`: N rooms for a seed, vacant until something connects as one.
...
/// `kickoff-channel adopt-secrets [--apply]`: copy every enrolled project's secret to where the channel
    ...
    if copied == 0 && linked == 0 {
        ...
        writeln!(
            ...
            "Each repo's own copy is left where it was. Take one away, per project, once you are \
             ready:  kickoff-channel remove-repo-secret <repo>"
        )?;
...
const ONLY_THIS_BUILD: &str = "From now on, rotate a secret only with this build of \
    kickoff-channel — an older one rewrites the repo's copy alone and leaves the channel's behind, \
    and every new session then presents the stale one. Install this build everywhere your shell \
    finds the command, and run the hub on it.";

/// `kickoff-channel remove-repo-secret <repo>`: take the repo's copy of the secret away, once the
...
fn remove_in(
    ...
    let Some(channel_copy) = home.read_secret(&p.id)? else {
        anyhow::bail!(
            "the channel holds no copy of {}'s secret yet, so removing the repo's would take it \
             off the air. Copy it across first:  kickoff-channel adopt-secrets --apply",
            p.title
        );
    };
    ...
    if channel_copy.trim() != repo_copy {
        anyhow::bail!(
            "the channel's copy of {}'s secret is not the same bytes as the repo's, so removing \
             ...
             kickoff-channel adopt-secrets --apply",
            p.title
        );
    }
    ...
    if home.linked(&p.repo)?.as_ref() != Some(&p.id) {
        anyhow::bail!(
            "{}'s repo is not linked to its conversation, so a session started there would find \
             nothing once the repo's copy goes. Link it first:  kickoff-channel adopt-secrets --apply",
            p.title
        );
    }
    ...
    writeln!(
        ...
        "\nWhat this cannot tell: whether a session already running has reconnected on the new \
         ...
         The way back is:  kickoff-channel enroll {}  — a rotation, which now writes the channel's copy \
...
mod tests {
    ...
    fn adopt_secrets_on_a_box_with_nothing_enrolled_says_so_rather_than_a_finished_migration() {
        ...
        assert!(
            err.contains("nothing is enrolled") && err.contains("kickoff-channel projects"),
            "{err}"
        );
        assert!(
    ...
    fn removing_a_repo_secret_is_refused_until_the_channel_holds_the_same_bytes() {
        ...
        assert!(
            out.contains("kickoff-channel enroll"),
