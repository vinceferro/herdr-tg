...
//!
//! These two files are the source of truth for the hub's health. `kickoff-channel doctor` and the watchdog
...
impl Heartbeat {
    ...
    ///
    /// Only for `kickoff-channel doctor`. The watchdog does its own reading, on purpose: a hub that can
...
mod tests {
    ...
    fn the_default_path_is_the_one_the_watchdog_unit_names() {
        // Pinned against the literal in deploy/kickoff-channel-watchdog.service. If either moves, the hub
