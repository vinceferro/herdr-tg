...
impl std::fmt::Display for LockError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            //
            // BOTH units are named, and naming one would be worse than wordy. This lock is taken
            // before the plane is chosen and both planes take it, so the copy already holding it
            // may be either — and the lock file carries a pid and nothing else, so which unit
            // started it is genuinely unknown at this point. Guessing from THIS process's own
            // plane would print the wrong unit in exactly the case worth printing: a phone hub
            // refused by an app hub that is already up. A name he pastes into `systemctl` that is
            // not loaded leaves him with nothing to try, which is the failure
            // `every_unit_this_product_tells_him_to_act_on_is_one_an_installer_here_lays_down`
            // exists to prevent. Naming two units costs two of them being INSTALLED, not merely
            // shipped, and that guard holds these two sentences to what `scripts/install-service.sh`
            // really writes into his unit directory — which is why that script now lays both planes
            // down and starts one.
            Self::Held { pid: Some(pid), .. } => write!(
                f,
                "another copy of this bridge is already running (process {pid}), and only one can \
                 hold this box's line to you. Stop that one first, whichever way it reaches you:  \
                 systemctl --user stop kickoff-channel-app  (through the app), or  \
                 systemctl --user stop kickoff-channel  (through Telegram)"
            ),
            Self::Held { pid: None, .. } => write!(
                f,
                "another copy of this bridge is already running, and only one can hold this box's \
                 line to you. It started moments ago and has not named itself yet; try again in a \
                 second, or stop it, whichever way it reaches you:  \
                 systemctl --user stop kickoff-channel-app  (through the app), or  \
                 systemctl --user stop kickoff-channel  (through Telegram)"
            ),
            Self::Io { path, source } => {
