...
//! it never names something new. The only way in is `kickoff-channel enroll <repo>`, run by a human at a
//! keyboard — the only way off, or back on, is `kickoff-channel disable <repo>` / `enable <repo>` at the
//! same keyboard — and the only way a person is let into a project's conversations is
//! `kickoff-channel allow <repo> <user>` there too. `nothing_inbound_can_add_a_person.rs` pins that the
...
pub struct Project {
    ...
    pub title: String,
    /// The canonical repo path. For the audit record and for `kickoff-channel projects`.
    pub repo: PathBuf,
    ...
    /// in every other project's. Set only by `kickoff-channel allow <repo> <user>` at a terminal, and
...
impl Registry {
    ...
    ///
    /// The registry is not the hub's private state: `kickoff-channel enroll` writes it from a terminal
    ...
    /// Every project, for `kickoff-channel projects` and for the digest.
    ...
    /// row whenever there is one, so `kickoff-channel disable <repo>` matched by path alone would have
    ...
    ///
    /// Two processes write this file — the running hub, binding topics, and `kickoff-channel enroll` at a
    ...
    fn save(&self) -> Result<(), EnrolError> {
        ...
        // A temp name per process. Both writers — the running hub and `kickoff-channel enroll` at a
...
mod tests {
    ...
    fn switching_a_project_nobody_enrolled_is_refused_in_plain_words_rather_than_enrolling_it() {
        ...
        assert!(said.contains("kickoff-channel projects"), "{said}");
    ...
    fn a_folder_inside_an_enrolled_project_cannot_become_a_second_project() {
        ...
        assert!(said.contains("kickoff-channel enroll"), "{said}");
    ...
    fn switching_a_project_by_repo_switches_its_seed_and_every_room_of_it_and_a_room_by_id_only_itself()
     {
        ...
        // first row in id order, which a `c-` id sorts BEFORE a `p-` one — so `kickoff-channel disable
    ...
    fn opening_a_project_whose_channel_copy_is_stale_says_so_and_names_the_verb_that_mends_it() {
        ...
        let said = r.open(&repo).expect_err("refused").to_string();
        assert!(
            said.contains("kickoff-channel enroll"),
