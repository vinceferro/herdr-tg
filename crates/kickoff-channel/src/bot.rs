...
//! written only by `kickoff-channel allow` at a keyboard). A command is answered with facts about every
...
//! # Why long-poll
//!
//! The bridge dials out and binds nothing. There is no listening port in Kickoff Channel at all, so the
...
async fn refused_sender(
    ...
    tracing::warn!(
        ...
        "from a person NOT allowed to speak here — ignored. To let this person into one \
         project's conversations: kickoff-channel allow <repo> <user id>. To let them speak anywhere \
...
#[command(
    ...
    description = "Kickoff Channel — your herd, from your pocket."
)]
pub enum Command {
    #[command(
...
fn tell_the_health_what_telegram_refused(health: &Health, error: &RequestError, now: Instant) {
    match error {
        // The conflict item 4 was written for. A second `kickoff-channel serve` beside the unit — a stray
...
/// exited. `Restart=always` with `RestartSec=5` in `deploy/kickoff-channel.service` is what keeps it
        ...
        Some(forum) => {
            ...
            match crate::transport::LocalSocket::bind(&sock) {
                ...
                Ok(socket) => {
                    ...
                    // The other half of `kickoff-channel disable`: the flag alone turns away the NEXT
...
async fn the_app_plane(
    ...
    // The other half of `kickoff-channel disable`, and the sweep that reads his answers out of the drop:
    ...
    if gate.nobody_may_speak() {
        tracing::warn!(
            "NOBODY may speak to this bot: no private chat is on the chat allowlist to name a \
             ...
             one project with `kickoff-channel allow <repo> <user>` can be heard there. Add your own \
        ...
        if from_private_chats.len() > 1 {
            tracing::warn!(
                ...
                "is a private chat on the chat allowlist, so this person may type at EVERY agent \
                 ...
                 project, take the chat off HERDR_TG_ALLOWED_CHAT_IDS and run: kickoff-channel allow \
                 <repo> {id}"
            );
        } else {
            tracing::info!(
                    ...
                    if hub.is_switched_off(who).await {
                        escape_html(
                            "This project is switched off, so nothing was sent. It will not be \
                             delivered later. Switch it back on at a terminal with:  \
                             kickoff-channel enable <its folder>",
                        )
                    } else {
                        escape_html(
...
pub(crate) async fn digest_of<S: crate::hub::Surface>(hub: &crate::hub::Hub<S>) -> String {
    ...
    let mut lines: Vec<String> = {
        ...
        // Fresh, because `kickoff-channel enroll` runs at a terminal while the hub is running. Rendered
    ...
    if lines.is_empty() {
        return escape_html(
            "Nothing is enrolled yet. Enrol a project at the terminal with: kickoff-channel enroll <repo>",
        );
    }
    lines.join("\n")
}
...
mod tests {
    ...
    async fn the_journal_names_a_refused_sender_by_the_number_or_as_unknown() {
        ...
        // `kickoff-channel allow <repo> <id>`. It printed `user=Some(999001)` and `user=None` — not a
    ...
    fn every_private_chat_on_the_chat_allowlist_is_announced_as_a_person_who_may_speak_anywhere() {
        ...
        for id in [OPERATOR, GUEST] {
            ...
            assert!(
                line.contains(&format!("kickoff-channel allow <repo> {id}")),
    ...
    /// A stray second `kickoff-channel serve` beside the unit is the likeliest way this box loses its
