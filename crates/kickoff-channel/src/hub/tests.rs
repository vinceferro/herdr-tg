...
async fn the_real_plugin_finds_its_project_when_the_session_started_in_a_folder_deep_inside_it() {
    ...
    let deep = repo.join("crates/kickoff-channel/src");
...
/// The harness project's folder, as the terminal would name it to `kickoff-channel allow`.
...
async fn a_person_let_in_at_the_terminal_is_heard_within_a_second_and_shut_out_as_fast() {
    // `kickoff-channel allow` is a registry write from another process. The hub answers "may this person
