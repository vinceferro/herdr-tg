<!-- SURFACE, v23, 18 September 2026. v23 adds one offer and moves nothing on the wire — offer 10
     is over HTTP and two files, not over hub-proto, which is exactly why it is an offer of this
     repo's whole product rather than of the hub's socket: the door gateway (`kickoff-door`) is a
     separate binary that binds loopback only, reads the ring this hub already appends and writes
     the answers drop this hub already sweeps, and serves the PWA org's own pinned /v1 shapes
     byte-compatibly. A bridge reading this file to plan an adapter changes nothing it does today:
     no frame, no field, no secret crosses the door.

     v22 corrects v21 in one offer and adds nothing to the
     wire. Offer 8 gave a seed re-enrolled at a MOVED folder as an example of a room that names
     itself, and it is not one: enrolling elsewhere leaves the old row on file, so the room goes on
     naming the row it was granted from. What the field really does in that case is written down
     now, and OPEN 3 carries the migration that ends it — which this makes the blocking half of,
     rather than an implementation detail, because two surfaces now publish the relation.

     v21 gives a conversation a NAME on the wire. The
     `welcome` now carries two ids — the conversation the secret resolved to, and the project it
     belongs to — and the read-only inventory carries a tenth field, `seed`, saying which project a
     room belongs to. Together they answer the one question OPEN 3 was left open on: what two
     programs join their records on. It is the ids, and never a path, so OPEN 3 is CLOSED and
     `docs/ATTACHING.md` §3b is the table of which of the things an adapter handles are fleet
     identity and which are one machine's business. Nothing already on the wire moved: both fields
     are omitted by a hub older than this, and `hello`'s `repo` and `pid` became optional because
     the hub has never routed on either.

     v20 corrects answer 6 and moves nothing on the wire.
...
**The command a person types is `kickoff-channel`.** It is installed as `herdr-tg` as well — the
same program under the name it shipped under first, kept because other software on that box calls
it — and either spelling runs any verb named below. The hub's state directory keeps the older name
for the same reason: a path other programs read does not move for a rename.
...
| --- | --- | --- |
| 1 | **A conversation of its own, and a name for it.** A forum topic per address, created on first LIVE connection and greeted so it appears in the list. **And the `welcome` says which conversation you are**: the id the secret resolved to, and the id of the project it belongs to — opaque, stable, and the only two things another program may join its records on. A path is not one of them. Both are omitted by a hub older than 8 September, and absence means what it always means: you were not told, so derive nothing. | Connect with a secret and an address; read `conversation` and `project_id` off the `welcome` (`docs/ATTACHING.md` §3b). |
...
| 10 | **A door for a reader that is not Telegram.** *Built 18 September, as a separate binary.* `kickoff-door` binds 127.0.0.1 only — the one program in this workspace that listens, ever — and serves over HTTP what the hub already writes to files: the ring of operator-visible events, as `GET /v1/stream` (SSE: `retry: 3000`, one `id:`+`data:` block per ring line, `: ping` when idle, replay from `Last-Event-ID` or `?cursor=`) and `GET /v1/events?cursor=N` (the same envelopes as one JSON object, `cursor` the echo of the last seq served and the ring's true head when nothing was served, every served event strictly the next seq past the cursor you asked with); and the answers drop, through `POST /v1/commands`. **This is not a second transport and not OPEN 4**: no hub-proto frame crosses it, nothing holds a claim through it, and a peer elsewhere still needs what OPEN 4 still is. The write door demands `Authorization: Bearer` against a token read from `<state>/door/token` per request — constant-time compared, rotation live, minted only at the terminal by `kickoff-channel door-token`, which refuses to overwrite silently and takes rotation anchored to the old token's first characters. A wrong or missing token is one byte-identical `401`, the shape your own bridge's tests pin. A command becomes a file in the drop (conversation named by the body, or by the door's own default, or resolved from the question it answers — never guessed) and the POST is answered from the hub's own `.result` file: accepted, or refused with the hub's sentence, and if no result lands inside two and a half seconds the answer is a `504` that claims nothing. **The door never answers optimistically, never mints a credential, and binds nothing but loopback** — there is no flag for another address, and the ring's egress law (no path, no chat id, no token in anything served) is asserted against the door's raw bytes, not assumed to carry over. The shapes are transcribed from the PWA org's own pinned stub, in their repo, because their client was written against it and byte-compat is the adoption's whole cost advantage; where their stub and a slow real hub differ, the extension is the timeout shape and it is documented as ours. | Run `kickoff-door` beside the hub (it finds the state home the same way every part of this product does); mint the token at the keyboard; point the PWA bridge's `HUB_URL` at `http://127.0.0.1:<port>`. |
...
1. **Enrolment, at a terminal, per conversation.** Admission is the one thing no message can do.
   ...
   is nothing in a working tree for git to commit. `kickoff-channel open <repo>` mints a project's
   conversation and writes nothing into the repo; `kickoff-channel grant <repo> --rooms N` mints N rooms
   ...
   `kickoff-channel enroll <repo>`, is also the rotation: it rewrites the secret where it already lives
   ...
   to its tree. `kickoff-channel adopt-secrets --apply` copies every repo-held secret across once, and
   `kickoff-channel remove-repo-secret <repo>` takes a repo's copy away, per project, at the operator's
   ...
   The switch is part of admission: `kickoff-channel disable <repo>` turns the project off — its seed
...
5. **Who may speak, said at a terminal.** The chat allowlist says WHERE the bot listens and
   ...
   a customer, a teammate in one room — are let in with `kickoff-channel allow <repo> <user>` and heard
...
An item that has been answered keeps its number and says so where it stands, because other documents
and both orgs' notes cite these numbers; renumbering the list to tidy it would silently repoint every
one of those citations at somebody else's question.

1. **Who starts a container.** Nothing the hub reads can reach a process today — the binary's only
   three shipped `Command` call sites are enrolment's, on `argv` at the terminal, naming `git` as a
   literal (`nothing_inbound_can_start_a_process.rs`) — and seam ④ of
...
3. **The room-map handshake. CLOSED, 8 September, by the ids.** The counter-proposal stands
   whole except for its last clause: the repo file is entirely yours (room name, memory scope,
   charter, engine), topic ids stay in our registry and are exposed read-only, and it is still true
   that the hub writes into no repo and that a topic id does not exist at enrol time. What was open
   was the **join key**, proposed as the repo path. It is not the path. It is the **conversation
   id** — the string a terminal printed when the room was granted, which whoever dispatches already
   hands the worker as `KICKOFF_HUB_CONVERSATION`, and which the `welcome` now names back so both
   ends can prove they mean the same conversation — with `seed` (offer 8) relating a room to the
   project it belongs to. A path could not do this job: a room's `repo` is its seed's, so several
   rooms carry one string; a wall mounts a checkout somewhere else; and a moved checkout is a
   different path for the same work. `docs/ATTACHING.md` §3b is the whole table of what may be
   joined on. Two things this does NOT settle, said plainly rather than left to be assumed: what a
   room needs that a lane does not is still OPEN 2, and a seed's id is minted today by hashing its
   enrolment path, so a project re-enrolled at a moved path is a new id — an implementation detail
   nobody may reproduce, and paying it off (random minting, the id stored on the row) changes
   nothing in this answer. That last half stopped being merely named on 8 September and is the
   blocking half of this item now, because two surfaces publish the relation and both read it off
   the shared path: a room whose seed moved keeps naming the row at the old folder, and a project
   later enrolled at the folder the first one vacated takes the same id back and inherits its rooms
   along with its topic. Neither is announced anywhere. Storing the seed's id on a room's row at
   `grant` is what ends both, and until it is done the relation is honest only while a project
   stays where it was enrolled.
...
6. **How a dispatcher reads this hub's health.** Offer 7 turns the alarm into a statement about the
   ...
   reading rather than asking the hub, and why `kickoff-channel doctor` reports the stamp by opening the
   ...
   command** — `kickoff-channel doctor --json`, which carries how long ago the hub stamped, whether the
