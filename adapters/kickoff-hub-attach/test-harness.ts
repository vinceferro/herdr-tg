...
// The fake opencode server and its session fixture live in `fake-opencode.ts` and are re-exported
// here, because a fixture PROCESS has to start one without importing this file: the line above
// points `XDG_STATE_HOME` somewhere of its own on import, and a child that took its state home
// from that would read a channel home its parent never chose. Re-exported rather than left behind,
// so every suite that already imports them from here keeps working and there is still one copy.
export { aSession, fakeOpencode } from './fake-opencode.ts'
